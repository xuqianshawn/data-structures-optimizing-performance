...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) {
		...
		int numSuggestions = 1000000; // the same
		// string multiple
		// times
		...
		while (queue.size() > 0) {
			WPTreeNode currentword = queue.poll();
			List<String> oneMutations = nw.distanceOne(currentword.getWord(), true);
			for (String w : oneMutations) {
				if (!visited.contains(w)) {
					visited.add(w);
					WPTreeNode child=currentword.addChild(w);
					queue.add(child);										
					numSuggestions--;
                    if(w.equals(word2))
                    {
